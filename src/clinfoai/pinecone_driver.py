...
class PineCone_Driver:
    def __init__(
        self, model, index_name, key, env, embed_dim: int = None, metric="cosine"
    ):
        self.model = model
        if embed_dim is None:
            try:
                ...
                raise ValueError(
                    "embed_dim is not provided and model does not have get_sentence_embedding_dimension() method, please provide embed_dim"
                )
    ...
    def _get_index(self):
        if self.index_name not in pinecone.list_indexes():
            pinecone.create_index(
                name=self.index_name, dimension=self.embed_dim, metric=self.metric
            )
    ...
    def get_most_similar(
        self, query, top_k: int = 10, include_metadata=True, threshold: float = 0.80
    ) -> list:
        result = self.semantic_query(
